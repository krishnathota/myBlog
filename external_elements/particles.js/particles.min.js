var pJS=function(tag_id,params){var canvas_el
if(typeof(tag_id)!='string'&&tag_id instanceof Element){canvas_el=tag_id.querySelector('.particles-js-canvas-el')}else{canvas_el=document.querySelector('#'+tag_id+' > .particles-js-canvas-el')}
...
window.pJSDom=[];window.particlesJS=function(tag_id,params){var pJS_tag;if(typeof(tag_id)!='string'&&tag_id instanceof Element){pJS_tag=tag_id}else if(typeof(tag_id)!='string'){params=tag_id;tag_id='particles-js';pJS_tag=document.getElementById(tag_id)}else{if(!tag_id){tag_id='particles-js'}
pJS_tag=document.getElementById(tag_id)}
pJS_canvas_class='particles-js-canvas-el',exist_canvas=pJS_tag.getElementsByClassName(pJS_canvas_class);if(exist_canvas.length){while(exist_canvas.length>0){pJS_tag.removeChild(exist_canvas[0])}}
var canvas_el=document.createElement('canvas');canvas_el.className=pJS_canvas_class;canvas_el.style.width="100%";canvas_el.style.height="100%";var canvas=pJS_tag.appendChild(canvas_el);if(canvas!=null){pJSDom.push(new pJS(tag_id,params))}};window.particlesJS.load=function(tag_id,path_config_json,callback){var xhr=new XMLHttpRequest();xhr.open('GET',path_config_json);xhr.onreadystatechange=function(data){if(xhr.readyState==4){if(xhr.status==200){var params=JSON.parse(data.currentTarget.response);window.particlesJS(tag_id,params);if(callback)callback()}else{console.log('Error pJS - XMLHttpRequest status: '+xhr.status);console.log('Error pJS - File config not found')}}};xhr.send()}